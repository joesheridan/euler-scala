...
val fractions = (1 to 1000).toList map (d => (d, new java.math.BigDecimal(1).divide(new java.math.BigDecimal(d.toString()), 1200, java.math.RoundingMode.CEILING)))

val repeatgroups = fractions.filter(a => {
  val len = a._2.toString().drop(2).length
  len > 30
}).map{ case (d, num) => {
  var digits = num.toString().drop(2).dropRight(1)
  (d, digits, getRepeatGroup(digits))
}}.map( triple => (triple._1, triple._2, triple._3.length) )
