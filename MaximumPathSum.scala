...
object MaximumPathSum extends App {
  ...
  def getNextPath(path:List[Int]): List[Int] = {
    ...
    return leftPad(nextPath, pathLength)
  }
   ...
   */
  def isMaximum(path:List[Int]) : Boolean = {
    return (path.sum == path.length)
  }
  
  var curPath = List.fill(14)(0)
  var maxVal = 0
  var maxPath:List[Int] = List()
  
  // loop until the path has reach the maximum
  while(!isMaximum(curPath)) {
    curPath = getNextPath(curPath)
    println(curPath)
    val pathVal = getPathVal(curPath, triangle)
    println("pathtotal:"+pathVal)
    if (pathVal > maxVal) {
      maxVal = pathVal
      maxPath = curPath
    }
  }
  println("maximal path: "+maxVal)
